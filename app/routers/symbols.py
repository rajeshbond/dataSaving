...
#         data_csv['nsecode'].fillna('NA', inplace=True)  # Replace NaN in nsecode with 'NA'
#         data_csv['bsecode'].fillna(0, inplace=True)  
#     # Assuming "Symbol" is the only column (adjust for other columns)
#         symbol_objects = data_csv.apply(lambda row: models.Symbol(**row.to_dict()), axis=1).tolist()
#         print(symbol_objects)
#         data_csv['nsecode'].fillna('NA', inplace=True)  # Replace NaN in nsecode with 'NA'
#         data_csv['bsecode'].fillna(0, inplace=True)  # Replace NaN in bsecode with 0
...
#                 return {"message": "No new symbols to add"}
#         else:
#             print("Symbols table is not empty")
#             return {"message": "Symbols table is not empty"}
#     except Exception as e:
#         db.rollback()
#         raise HTTPException(status_code=500, detail=str(e))